...
export default function ProfileSection() {
  ...
  useEffect(() => {
    axios
      .get(
        import.meta.env.VITE_BACKEND_API + "/v1/student/profile/personalInfo",
    ...
    axios
      .get(
        import.meta.env.VITE_BACKEND_API + "/v1/student/profile/contactdetails",
        { withCredentials: true }
      )
      .then((res) => {
    ...
    axios
      .get(
        import.meta.env.VITE_BACKEND_API + "/v1/student/profile/parentinfo",
        { withCredentials: true }
      )
      .then((res) => {
  ...
  return (
    <div className="bg-[#ffffff] h-[75vh] max-[1024px]:w-[94%] max-[1024px]:ml-[100px] max-[1024px]:mx-0 max-[767px]:translate-x-5 rounded-3xl mx-4 mt-4 overflow-y-scroll overflow-x-hidden">
      ...
      {activeoption === 1 && (
        <ContactInfo personalData={personalData} contactData={contactData} />
      )}
      {activeoption === 2 && (
        <ParentsInfo personalData={personalData} parentsData={parentsData} />
      )}
      {activeoption === 3 && <Education />}
      {activeoption === 4 && <Awards />}
      {activeoption === 5 && (
